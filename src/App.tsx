              ...
              {!timer.isStarted ? (
                <motion.div key="countdown" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -20 }} className="bg-brand-green rounded-4xl p-7 text-white shadow-xl relative overflow-hidden mx-1">
                   <div className="relative z-10 flex justify-between items-center">
                     <div>
                       ...
                       <div className="w-32 h-2 bg-black/10 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={!isLoading ? { width: `${timer.percent}%` } : { width: 0 }} transition={{ duration: 4, ease: "easeOut" }} className="h-full bg-white/50 rounded-full" /></div>
                     </div>
                     <div className="text-right leading-none">
                       <div className="text-5xl font-black italic tracking-tighter flex items-baseline justify-end"><CounterNumber target={timer.h!} isReady={!isLoading} /><span className="text-xl ml-1 not-italic opacity-80 font-black">小時</span></div>
                       <p className="text-[10px] font-black opacity-70 tracking-widest uppercase mt-2">剩餘 {timer.m}分 {timer.s}秒</p>
                     </div>
                   </div>
                ...
                <motion.div key="live" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[2.5rem] p-5 shadow-sticker border-2 border-brand-yellow flex items-center justify-between mx-1">
                  <div className="flex items-center gap-4">
                    ...
                    <div><p className="text-[10px] font-black text-brand-yellow uppercase tracking-widest">下個行程 NEXT</p><h3 className="text-xl font-black text-brand-brown">{currentEvent ? currentEvent.title : "行程結束囉！"}</h3>{currentEvent && <p className="text-xs font-bold text-gray-400 mt-0.5">{currentEvent.time} 開始</p>}</div>
                  </div>
