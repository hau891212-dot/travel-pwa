      ...
      <AnimatePresence>
        {isLoading && (
          ...
          >
            ...
            <div className="mt-12 bg-white px-8 py-3 rounded-full shadow-sticker border-2 border-[#E0E5D5] font-black text-brand-brown tracking-[0.2em] uppercase text-sm">
              雲端資料同步中...
            </div>
          </motion.div>
        )}
      ...
      <div className="sticky top-0 z-50 bg-brand-bg/70 backdrop-blur-xl border-b border-[#E0E5D5]/30">
        <header className="p-6 pb-2 flex justify-between items-start">
          ...
          <div className="flex -space-x-3 pt-1">
            {[1, 2, 3, 4].map(i => (
              <img key={i} className="w-10 h-10 rounded-full border-2 border-white shadow-sm bg-gray-200" src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${i + 88}`} alt="avatar" />
            ))}
          </div>
        </header>

        {/* 只有在行程分頁顯示日期，且會跟著 Header 一起固定 */}
        <AnimatePresence>
          {activeTab === 'schedule' && (
            <motion.div 
              ...
              {weekList.map((item, i) => (
                <button
                  ...
                  className={`flex-shrink-0 w-16 h-20 rounded-2xl flex flex-col items-center justify-center transition-all duration-300 ${
                    selectedDay === i ? 'bg-brand-blue text-white shadow-lg scale-105' : 'bg-white text-gray-400 border border-[#E0E5D5]/50 shadow-sm'
                  }`}
                >
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-brand-green rounded-4xl p-7 text-white shadow-xl shadow-green-100/50 relative overflow-hidden">
               <div className="relative z-10">
                 ...
                 <div className="w-40 h-2.5 bg-black/10 rounded-full overflow-hidden mb-2">
                    <motion.div 
                      ...
                      className="h-full bg-white/50 rounded-full shadow-inner"
                    ></motion.div>
                 </div>
               </div>
               ...
               <div className="absolute right-7 bottom-6 text-right z-10">
                 ...
                 <p className="text-[10px] font-black opacity-70 mt-1 uppercase">
