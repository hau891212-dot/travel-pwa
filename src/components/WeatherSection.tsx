...
import { faCloudRain, faWind, faSun, faTshirt, faCloud } from '@fortawesome/free-solid-svg-icons';
...
export const WeatherSection = () => {
  ...
  if (loading) return <div className="mx-2 h-64 bg-white/30 animate-pulse rounded-4xl"></div>;
      ...
      <div className="flex justify-between items-start relative z-10">
        <div>
          <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest flex items-center gap-1">
            <span className="w-2 h-2 bg-green-300 rounded-full animate-ping"></span> 大阪即時天氣
          </p>
        ...
        <div className="bg-white/20 backdrop-blur-md p-4 rounded-3xl text-right border border-white/10 shadow-inner">
          ...
          <div className="text-[10px] font-bold opacity-80">體感 {Math.round(weather?.main.feels_like)}°</div>
        </div>
      </div>

      <div className="bg-white rounded-3xl mt-6 p-4 flex items-center gap-4 text-brand-brown shadow-lg relative z-10">
        <div className="bg-brand-yellow w-12 h-12 rounded-full flex-shrink-0 flex items-center justify-center text-white">
          <FontAwesomeIcon icon={faTshirt} />
        </div>
        <div>
          <p className="text-[10px] font-black text-brand-yellow uppercase">OUTFIT ADVICE 穿搭建議</p>
          <p className="text-xs font-bold leading-tight mt-0.5">{getAdvice(weather?.main.temp, weather?.weather[0].description)}</p>
        </div>
      </div>
